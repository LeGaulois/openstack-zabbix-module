...
(function() {
  ...
  describe('Launch Instance Zabbix Step', function() {

    describe('LaunchInstanceZabbixTemplatesController', function() {
      var ctrl;
      ...
      beforeEach(inject(function($controller) {
        var model = {
          newInstanceSpec: {
            zabbix_groups: [ 'zabbix template 1' ]
          },
          zabbixGroups: [ 'zabbix template 1', 'zabbix template 2' ]
        };
        ctrl = $controller(
          'LaunchInstanceZabbixTemplatesController',
          {
      ...
      it('sets table data to appropriate scoped items', function() {
        ...
        expect(ctrl.tableData.available).toEqual([ 'zabbix template 1', 'zabbix template 2' ]);
        expect(ctrl.tableData.allocated).toEqual([ 'zabbix template 1' ]);
      ...
      it('allows only one allocation', function() {
        ...
        expect(ctrl.tableLimits.maxAllocation).toBe(-11);
      });
    });
