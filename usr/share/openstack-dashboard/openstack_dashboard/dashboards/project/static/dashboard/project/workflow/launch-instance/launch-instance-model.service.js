...
(function () {
  ...
  launchInstanceModel.$inject = [
    ...
    'horizon.app.core.openstack-service-api.zabbix',
  ...
  function launchInstanceModel(
    ...
    function initialize(deep) {
      ...
      if (model.initializing) {
        ...
        promise = $q.all([
          ...
          zabbixAPI.getZabbixGroups(),
          zabbixAPI.getZabbixTemplates(),
    ...
    function getZabbixGroups() {
        return zabbixAPI.getZabbixGroups().then(onGetZabbixGroups, noop);
    }
    
    function getZabbixTemplates() {
        return zabbixAPI.getZabbixTemplates().then(onGetZabbixTemplates, noop);
    }

    function onGetZabbixGroups(data) {
      model.zabbixGroups.length = 0;
      push.apply(model.zabbixGroups, data.data.items);
      
      /*Adaptation du dictionnaire
      On remplace l'entrée gid par id
      Cela est necessaire pour assurer que les scripts JS
      de gestion d'affichage puissent suprimmer ou ajouter 
      correctement les differentes entrees des tables
      available et displayed
      */
      var len_zabbixgroups = model.zabbixGroups.length;
      var i   = 0;
      var gid = 0;
      
      for(i=0; i<len_zabbixgroups; i++){
        gid =   model.zabbixGroups[i]["groupid"];
        model.zabbixGroups[i]["id"]=gid;
        delete model.zabbixGroups[i]["groupid"];
      }
    }
    
    function onGetZabbixTemplates(data) {
      model.zabbixTemplates.length = 0;
      push.apply(model.zabbixTemplates, data.data.items);
      
      /*Adaptation du dictionnaire
      On remplace l'entrée templateid par id
      Cela est necessaire pour assurer que les scripts JS
      de gestion d'affichage puissent suprimmer ou ajouter 
      correctement les differentes entrees des tables
      available et displayed
      */
      var len_zabbixtemplates = model.zabbixTemplates.length;
      var i   = 0;
      var templateid = 0;
      
      for(i=0; i<len_zabbixtemplates; i++){
        templateid =   model.zabbixTemplates[i]["templateid"];
        model.zabbixTemplates[i]["id"]=templateid;
        delete model.zabbixTemplates[i]["templateid"];
      }
    }

    function setFinalSpecZabbix(finalSpec) {      
      var zbxhost = {};
      zbxhost["name"]=model.newInstanceSpec.name;
      zbxhost["groupid"]=[];
      zbxhost["templateid"]=[];
      
      finalSpec.zabbix_groups.forEach(function (zbxgroup) {
        zbxhost.groupid.push(Number(zbxgroup.id));
      });

      finalSpec.zabbix_templates.forEach(function (zbxtemplate) {
        zbxhost.templateid.push(Number(zbxtemplate.id));
      });

      delete finalSpec.zabbix_groups;
      delete finalSpec.zabbix_templates;      
      zabbixAPI.setZabbixHost(zbxhost);
    }
    //END ZABBIX
